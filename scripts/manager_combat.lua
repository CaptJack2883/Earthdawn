...
function ctRest()
  print("Starting Rest Function.");
  for _,v in pairs(CombatManager.getCombatantNodes()) do
    --print("Found Combatant, checking if they are a PC.");
    local rActor = ActorManager.resolveActor(v);
    --print("rActor.sName");
    --print(rActor.sName);
    if ActorManager.isPC(rActor) then
      --somehow, we lost george?
      print("Found PC, Starting Rest Function.");
      local rActor = ActorManager.resolveActor(v);
      --print(rActor.sname);
      local sClass, sRecord = DB.getValue(v, "link", "", "");
			local nodeActor = DB.findNode(sRecord);
      
      print(nodeActor.sname);
      -- updateKarma is nil??
      --rActor.main.updateKarma();
      --print(nodeActor.sname);
      -- Need to use internal function to perform karma reset.
      CharacterManager.overnightRest(nodeActor);
    end
	end  
  print("Ending Rest Function.");
end
